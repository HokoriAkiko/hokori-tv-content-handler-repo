...
const MissingInfo = (props)=>{
    ...
    useEffect(()=>{
        const get_count = async ()=>{
            try{
                const {result} = await fetch(`${env.REACT_APP_STORAGE_URL}/missing_infos`).then(resp=>resp.json());
                set_count(result);
                handle_animation(0,result,result * 100);
            }
            catch(e){toast("Error in Getting Missing Infos Count",{type: "error"});}
        }
    ...
    return(
        <button className="card lack column_box flex_1_box" title="Click to View">
            ...
            <div className="message">Series have missing information</div>
        </button>
    )
}
